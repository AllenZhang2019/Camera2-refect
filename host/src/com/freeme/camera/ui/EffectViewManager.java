...
class EffectViewManager extends AbstractViewManager {
    ...
    public void setEnabled(boolean enabled) {
        ...
        if (mViewEntry != null) {
            mViewEntry.setEnabled(enabled);
        }
    }
    /**
     * Set nine grid view entry.
     *
    ...
    /**
     * Attach nine grid view entry to view tree.
     */
    public synchronized void attachViewEntry() {
        mEffectViewGroup.removeAllViews();
        if (mViewEntry != null) {
            int orientation = mApp.getGSensorOrientation();
            CameraUtil.rotateRotateLayoutChildView(mApp.getActivity(), mViewEntry,
                    orientation, false);
            mEffectViewGroup.addView(mViewEntry);
        }
    }
}
