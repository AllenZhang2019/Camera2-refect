...
import com.freeme.camera.common.IAppUiListener.OnGestureListener;
import com.freeme.camera.common.IAppUiListener.OnModeChangeListener;
import com.freeme.camera.common.IAppUiListener.OnPreviewAreaChangedListener;
import com.freeme.camera.common.IAppUiListener.OnPreviewTouchedListener;
import com.freeme.camera.common.IAppUiListener.OnShutterButtonListener;
import com.freeme.camera.common.IAppUiListener.OnThumbnailClickedListener;
...
import com.freeme.camera.common.mode.IReviewUI;
import com.freeme.camera.common.mode.photo.intent.IIntentPhotoUi;
import com.freeme.camera.common.mode.video.videoui.IVideoUI;
...
import com.freeme.camera.common.widget.RotateLayout;
import com.freeme.camera.gesture.GestureManager;
import com.freeme.camera.ui.modepicker.ModePickerManager;
import com.freeme.camera.ui.modepicker.ModeProvider;
import com.freeme.camera.ui.photo.IntentPhotoUi;
import com.freeme.camera.ui.preview.PreviewManager;
import com.freeme.camera.ui.shutter.ShutterButtonManager;
import com.freeme.camera.ui.video.VideoUI;
...
public class CameraAppUI implements IAppUi {
    ...
    List<ModeItem> mSupportitems = new ArrayList();
    ...
    public void onCreate() {
        ...
        //mEffectViewManager.setVisibility(View.GONE);
    ...
    public void registerMode(List<ModeItem> items) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ModeItem item = (ModeItem) items.get(i);
            if (true || this.mNormalType.equals(item.mModeName)) {
                this.mModeProvider.registerMode(item);
                if (!this.mIsCameraSwitch) {
                    this.mShutterManager.registerShutterButton(item, i);
                }
            }
        }
    ...
    public void updateCurrentMode(final String modeEntry) {
        ...
        if (this.mModeProvider != null) {
            ModeItem item = this.mModeProvider.getMode(modeEntry);
            if (item != null && !item.mModeName.equals(this.mCurrentModeName)) {
                this.mCurrentModeName = item.mModeName;
                this.mCurrentModeType = item.mType;
                this.mCurrentMode = item.mMode;
                String[] supportTypes = this.mModeProvider.getModeSupportTypes(this.mCurrentModeName, this.mCurrentCameraId);
                this.mModePickerManager.updateCurrentModeItem(item);
            }
        }
    }
    ...
    @Override
    public ViewGroup getModeRootPreviewView() {
        return (ViewGroup) mApp.getActivity()
    ...
    public void onCameraSelected(final String cameraId) {
        ...
        if (!(this.mModeProvider == null || this.mShutterManager == null)) {
            int i;
            this.mIsCameraSwitch = true;
            this.mApp.getActivity().runOnUiThread(new Runnable() {
                public void run() {
                    CameraAppUI.this.mShutterManager.clearShutterButton();
                }
            });
            List<ModeItem> items = this.mModeProvider.getAllModes();
            int count = 0;
            this.mSupportitems.clear();
            for (i = 0; i < items.size(); i++) {
                ModeItem item = (ModeItem) items.get(i);
                for (String equals : item.mSupportedCameraIds) {
                    if (equals.equals(cameraId)) {
                        this.mShutterManager.registerShutterButton(item, count);
                        this.mSupportitems.add(item);
                        count++;
                    }
                }
            }
            ...
            if (!(this.mSupportitems.isEmpty() || this.mCurrentMode == null)) {
                for (i = 0; i < this.mSupportitems.size(); i++) {
                    if (this.mCurrentMode.equals(((ModeItem) this.mSupportitems.get(i)).mMode)) {
                        this.mShutterManager.setChangIndex(i);
                        this.mApp.getActivity().runOnUiThread(new Runnable() {
                            public void run() {
                                CameraAppUI.this.mShutterManager.updateModeSupportType();
                            }
                        });
                    }
                }
    ...
    private class OnShutterChangeListenerImpl implements
        ...
        public void onShutterTypeChanged(String newModeName, String newModeType) {
            CameraAppUI.this.mCurrentModeType = newModeType;
            CameraAppUI.this.mCurrentModeName = newModeName;
            LogHelper.i(CameraAppUI.TAG, "onShutterTypeChanged mCurrentModeType " + CameraAppUI.this.mCurrentModeType + " mCurrentModeName = " + CameraAppUI.this.mCurrentModeName);
            ModeItem item = CameraAppUI.this.mModeProvider.getModeEntryName(CameraAppUI.this.mCurrentModeName, CameraAppUI.this.mCurrentModeType);
            CameraAppUI.this.mCurrentMode = item.mMode;
            CameraAppUI.this.mModeChangeListener.onModeSelected(item.mClassName);
            CameraAppUI.this.mShutterManager.updateCurrentModeShutter(item.mType, item.mShutterIcon);
        }
    }
    ...
    private void configUIVisibility(int module, int visibility) {
        ...
        switch (module) {
            ...
            case SCREEN_HINT:
                mOnScreenHintManager.setVisibility(visibility);
            case EFFECT_VIEW:
                mEffectViewManager.setVisibility(visibility);
            default:
                break;
        }
    }
    ...
    public PreviewManager getmPreviewManager() {
