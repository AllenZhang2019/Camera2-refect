...
VideoQualitySettingView.OnValueChangeListener {
    ...
    public void refreshViewEntry() {
        if (mSettingView != null) {
            String value = mDataStore.getValue(getKey(), getDefaultQuality(), getStoreScope());
    ...
    public void updateValue(String defaultValue) {
        mDataStore.setValue(KEY_DEFAULT_VIDEO_QUALITY, defaultValue, getStoreScope(), false);
    ...
    private String getDefaultQuality() {
        int defaultIndex = 0;
        if (getSupportedPlatformValues().size() > QUALITY_INDEX_NUMBER) {
            defaultIndex = 1;
        }
