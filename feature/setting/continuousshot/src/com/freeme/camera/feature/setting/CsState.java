    ...
    private void checkState(State oldState, State newState) {
        ...
        switch (newState) {
            case STATE_OPENED:
                ...
                if (oldState != State.STATE_INIT && oldState != State.STATE_SAVING
                        && oldState != State.STATE_CAPTURE_STARTED) {
                    LogHelper.e(TAG, "[checkState]Error!");
                }
                break;
            case STATE_CAPTURE_STARTED:
                // OPENED --> CAPTURE_STARTED when start continuous shot.
                if (oldState != State.STATE_OPENED) {
                    LogHelper.e(TAG, "[checkState]Error!");
                }
                break;
            ...
            case STATE_SAVING:
                ...
                if (oldState != State.STATE_STOPPED) {
                    LogHelper.e(TAG, "[checkState]Error!");
                }
                break;
            case STATE_INIT:
                // OPENED --> INIT when close() is called.
                // SAVING --> INIT in the time close() is called when in saving state.
                if (oldState != State.STATE_OPENED && oldState != State.STATE_SAVING) {
                    LogHelper.e(TAG, "[checkState]Error!");
                }
                break;
            default:
