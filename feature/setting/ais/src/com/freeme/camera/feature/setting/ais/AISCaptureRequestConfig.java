...
package com.freeme.camera.feature.setting.ais;
...
public class AISCaptureRequestConfig implements ICameraSetting.ICaptureRequestConfigure {
    ...
    public void setCameraCharacteristics(CameraCharacteristics characteristics) {
        ...
        for (int i = 0; i < availableKeys.size(); i++) {
            ...
            if (AIS_AVAILABLE_MODES_KEY_NAME.equals(key.getName())) {
                mAisAvailableModesKey = (CameraCharacteristics.Key<int[]>) key;
                break;
            }
        }
        ...
        for (int i = 0; i < availableRequestKeys.size(); i++) {
            ...
            if (AIS_REQUEST_MODE_KEY_NAME.equals(key.getName())) {
                mAisRequestModeKey = (CaptureRequest.Key<int[]>) key;
                break;
            }
        }
        ...
        for (int i = 0; i < availableResultKeys.size(); i++) {
            ...
            if (AIS_RESULT_MODE_KEY_NAME.equals(key.getName())) {
                mAisResultModeKey = (CaptureResult.Key<int[]>) key;
                break;
            }
        }
