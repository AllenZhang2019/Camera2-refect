 ...
 */
package com.freeme.camera.feature.setting.focus;
...
import com.freeme.camera.common.sound.ISoundPlayback;
import com.freeme.camera.common.utils.CameraUtil;
import com.freeme.camera.common.utils.CoordinatesTransform;
...
public class Focus extends SettingBase implements
    ...
    private static final String ROI_DEBUG_PROPERTY = "mtk.camera.app.3a.debug";
    ...
    private String mCurrentMode = "com.freeme.camera.common.mode.photo.PhotoMode";
    ...
    private static final String KEY_FOCUS_SOUND = "key_focus_sound";
    private String mPrevFocusSound = "on";
    private static final String KEY_TOUCH_CAPTURE = "key_touch_capture";
    private String mPrevTouchCapture = "on";
    ...
    public void updateModeDeviceState(String newState) {
        ...
        switch (newState) {
            ...
            case ICameraMode.MODE_DEVICE_STATE_PREVIEWING:
                // flash on->hdr on->do continue shot->hdr off->restartPreview,
                // mode device will be update to MODE_DEVICE_STATE_PREVIEWING, we should ignore
                // this update.
                if ("on".equals(mSettingController.queryValue(KEY_CSHOT))) {
                    LogHelper.w(mTag, "[updateModeDeviceState] ignore update continue shot is on");
                } else {
                    mNeedPlayFocusSound = true;
                    mNeedShowFocusUi = true;
    ...
    public boolean onSingleTapUp(float x, float y) {
        //if (mApp.getCurrentCameraMode() == FreemeSceneModeData.FREEME_SCENE_MODE_BV_ID) {
            //mFocusViewController.removeFocusView();
            //return false;
        //}
        ...
        String touchCapturede = mSettingController.queryValue(KEY_TOUCH_CAPTURE);
        if(mPrevTouchCapture.equals(touchCapturede)) {
    ...
    public boolean onLongPress(float x, float y) {
        ...
        if (mCurrentMode.equals("PanoramaMode")) {
            return false;
        }
        if (mCurrentMode.contains("com.freeme.camera.feature.mode.pip")) {
            return false;
        }
    ...
    public boolean onShutterButtonClick() {
        ...
        IFocusView.FocusViewState state = mFocusViewController.getFocusState();
        LogHelper.d(mTag, "[onShutterButtonClick] state " + state);
        if (IFocusView.FocusViewState.STATE_ACTIVE_FOCUSING == state &&
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case FOCUS_STATE_UPDATE:
                    onFocusStateUpdate((IFocusController.AutoFocusState) msg.obj);
                    break;
                default:
                    break;
            }
    ...
    private final IFocusController.FocusStateListener mFocusStateListener =
            new IFocusController.FocusStateListener() {
                ...
                @Override
                public void onFocusStatusUpdate(IFocusController.AutoFocusState state,
                                                long frameNumber) {
    ...
    private void onFocusStateUpdate(IFocusController.AutoFocusState state) {
            ...
            case ACTIVE_UNFOCUSED:
                ...
                if (mNeedTriggerShutterButton) {
                    ...
                    if (mNeedPlayFocusSound) {
                        ...
                        String volumeCapturede = mSettingController.queryValue(KEY_FOCUS_SOUND);
                        if(mPrevFocusSound.equals(volumeCapturede)) {
    ...
    private boolean needCancelAutoFocus() {
        ...
        IFocusView.FocusViewState state = mFocusViewController.getFocusState();
        boolean result = mFocusArea != null && (IFocusView.FocusViewState.STATE_ACTIVE_FOCUSING == state);
        if (result == false) {
