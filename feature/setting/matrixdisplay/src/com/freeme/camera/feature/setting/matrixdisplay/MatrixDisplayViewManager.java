 ...
 */
package com.freeme.camera.feature.setting.matrixdisplay;
...
/**
 * Matrix display view manager used to control the nine grid view
 * showing, hiding, destroy.
 */
...
public class MatrixDisplayViewManager {
    ...
    public interface ItemClickListener {
         ...
         */
        boolean onItemClicked(String effect);
    }
     ...
     *
     * @param callback The instance of {@link ViewStateCallback}.
     */
     ...
     *
     * @param listener The instance of {@link SurfaceAvailableListener}.
     */
     ...
     *
     * @param listener The instance of {@link EffectUpdateListener}.
     */
     ...
     *
     * @param listener The instance of {@link ItemClickListener}.
     */
    ...
    private int getEffectId(String effectName) {
        for (int i = 0; i < mEffectName.length; i++) {
            if (equals(effectName, mEffectName[i])) {
                LogHelper.d(TAG, "effectName:" + effectName + ", effetId:" + i);
                return i;
            }
        }
        LogHelper.d(TAG, "effectName:" + effectName + ", effetId: -1");
        return -1;
    }
    ...
    private class LomoSurfaceTextureListener implements TextureView.SurfaceTextureListener {
        ...
        public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
            LogHelper.d(TAG, "onSurfacetTextureAvailable(), surface:" + surface + ", width:"
                    + width + ", " + "height:" + height + ", mPosition:" + mPosition);
            mSurfaceList[mPosition] = new Surface(surface);
            if (mSurfaceAvailableListener != null) {
                mSurfaceAvailableListener.onSurfaceAvailable(mSurfaceList[mPosition],
                        width, height, mPosition);
            }
