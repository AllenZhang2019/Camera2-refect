 ...
 */
public class CameraPoseEntry extends FeatureEntryBase {
    private static final LogUtil.Tag TAG = new LogUtil.Tag(CameraPoseEntry.class.getSimpleName());
    private static final String MODE_ITEM_TYPE = "Picture";

    private static final int MODE_ITEM_PRIORITY = 80;
     ...
     */
    public CameraPoseEntry(Context context, Resources resources) {
    ...
    public boolean isSupport(CameraApi currentCameraApi, Activity activity) {
        ...
        if(!isThirdPartyIntent(activity) && ((CameraActivity)activity).mPluginManagerAgent.getModules().get(FreemeSceneModeData.FREEME_SCENE_MODE_POSE_ID) != null) {
            support = true;
        }
    ...
    public String getFeatureEntryName() {
        return CameraPoseEntry.class.getName();
    }
    ...
    public Object createInstance() {
        return new ExtrioPlugMode(FreemeSceneModeData.FREEME_SCENE_MODE_POSE_ID);
    }
    /**
    ...
    public ModeItem getModeItem() {
        ...
        modeItem.mModeName = mContext.getString(R.string.pose_dialog_title);
        modeItem.mSupportedCameraIds = new String[]{"0"};
        modeItem.mMode = "Pose";
        modeItem.mTitle = this.mContext.getResources().getString(R.string.pose_dialog_title);
        return modeItem;
    }
