...
import com.freeme.camera.common.device.CameraDeviceManager;
import com.freeme.camera.common.device.CameraDeviceManagerFactory.CameraApi;
import com.freeme.camera.common.device.CameraOpenException;
import com.freeme.camera.common.device.v1.CameraProxy;
import com.freeme.camera.common.setting.ISettingManager;
import com.freeme.camera.common.setting.ISettingManager.SettingDeviceRequester;
 ...
 */
public class ExtrioPlugDeviceController implements IExtrioPlugDeviceController, SettingDeviceRequester {
    private static final Tag TAG = new Tag(ExtrioPlugDeviceController.class.getSimpleName());
    ...
    private QrCodeScanHandler mRequestHandler;
     ...
     */
    ExtrioPlugDeviceController(@Nonnull Activity activity, @Nonnull ICameraContext context) {
        HandlerThread handlerThread = new HandlerThread("DeviceController");
        handlerThread.start();
        mRequestHandler = new QrCodeScanHandler(handlerThread.getLooper(), this);
    ...
    public void openCamera(DeviceInfo info) {
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.OPEN_CAMERA, info).sendToTarget();
    }

    @Override
    public Camera getCamera() {
        if (mRequestHandler.mCameraProxy != null) {
            return mRequestHandler.mCameraProxy.getCamera();
        } else {
            return null;
        }
    ...
    public void updatePreviewSurface(Object surfaceObject) {
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.UPDATE_PREVIEW_SURFACE, surfaceObject)
                .sendToTarget();
    }

    @Override
    public void setDeviceCallback(DeviceCallback callback) {
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.SET_PREVIEW_CALLBACK, callback)
                .sendToTarget();
    }

    @Override
    public void startPreview() {
        mRequestHandler.sendEmptyMessage(ExtrioPlugDeviceAction.START_PREVIEW);
    }

    @Override
    public void stopPreview() {
        mRequestHandler.sendEmptyMessage(ExtrioPlugDeviceAction.STOP_PREVIEW);
        waitDone();
    }

    @Override
    public void takePicture(@Nonnull JpegCallback callback) {
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.TAKE_PICTURE, callback).sendToTarget();
    }

    @Override
    public void updateGSensorOrientation(int orientation) {
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.UPDATE_G_SENSOR_ORIENTATION, orientation)
                .sendToTarget();
    }

    @Override
    public void closeCamera(boolean sync) {
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.CLOSE_CAMERA, sync == true ? 1 : 0)
                .sendToTarget();
        waitDone();
    }
    ...
    public Size getPreviewSize(double targetRatio) {
        ...
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.GET_PREVIEW_SIZE, values).sendToTarget();
        waitDone();
    ...
    public void setPreviewSizeReadyCallback(PreviewSizeCallback callback) {
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.SET_PREVIEW_SIZE_READY_CALLBACK,
                callback).sendToTarget();
    }

    @Override
    public void setPictureSize(Size size) {
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.SET_PICTURE_SIZE, size).sendToTarget();
    }

    @Override
    public boolean isReadyForCapture() {
        boolean[] isReady = new boolean[1];
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.IS_READY_FOR_CAPTURE, isReady)
                .sendToTarget();
        waitDone();
        return isReady[0];
    }
    ...
    public void requestChangeSettingValue(String key) {
        mRequestHandler.removeMessages(ExtrioPlugDeviceAction.REQUEST_CHANGE_SETTING_VALUE);
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.REQUEST_CHANGE_SETTING_VALUE, key)
                .sendToTarget();
    }
    ...
    public void requestChangeSettingValueJustSelf(String key) {
        ...
        mRequestHandler.removeMessages(ExtrioPlugDeviceAction
                .REQUEST_CHANGE_SETTING_VALUE_JUST_SELF, key);
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.REQUEST_CHANGE_SETTING_VALUE_JUST_SELF,
                key).sendToTarget();
    }

    @Override
    public void requestChangeCommand(String key) {
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.REQUEST_CHANGE_COMMAND, key).sendToTarget();
    }

    @Override
    public void requestChangeCommandImmediately(String key) {
        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.REQUEST_CHANGE_COMMAND_IMMEDIATELY, key)
                .sendToTarget();
    }

    @Override
    public void destroyDeviceController() {
        mRequestHandler.sendEmptyMessage(ExtrioPlugDeviceAction.DESTROY_DEVICE_CONTROLLER);
    }
     ...
     */
    private class QrCodeScanHandler extends Handler {
         ...
         */
        public QrCodeScanHandler(Looper looper, SettingDeviceRequester requester) {
            super(looper);
        ...
        public void handleMessage(Message msg) {
            ...
            if (cancelDealMessage(msg.what)) {
                LogHelper.d(TAG, "[handleMessage] - msg = " +
                        ExtrioPlugDeviceAction.stringify(msg.what) + "[dismiss]");
                return;
            }
            switch (msg.what) {
                case ExtrioPlugDeviceAction.OPEN_CAMERA:
                    doOpenCamera((DeviceInfo) msg.obj);
                    break;
                case ExtrioPlugDeviceAction.CLOSE_CAMERA:
                    doCloseCamera((Integer) msg.obj == 1 ? true : false);
                    break;
                case ExtrioPlugDeviceAction.START_PREVIEW:
                    doStartPreview();
                    break;
                case ExtrioPlugDeviceAction.STOP_PREVIEW:
                    doStopPreview();
                    break;
                case ExtrioPlugDeviceAction.TAKE_PICTURE:
                    doTakePicture((JpegCallback) msg.obj);
                    break;
                case ExtrioPlugDeviceAction.REQUEST_CHANGE_SETTING_VALUE:
                    ...
                    break;
                case ExtrioPlugDeviceAction.REQUEST_CHANGE_SETTING_VALUE_JUST_SELF:
                    ...
                    break;
                case ExtrioPlugDeviceAction.REQUEST_CHANGE_COMMAND:
                    doRequestChangeCommand((String) msg.obj);
                    break;
                case ExtrioPlugDeviceAction.REQUEST_CHANGE_COMMAND_IMMEDIATELY:
                    doRequestChangeCommandImmediately((String) msg.obj);
                    break;
                case ExtrioPlugDeviceAction.SET_PICTURE_SIZE:
                    doSetPictureSize((Size) msg.obj);
                    break;
                case ExtrioPlugDeviceAction.SET_PREVIEW_CALLBACK:
                    mModeDeviceCallback = (DeviceCallback) msg.obj;
                    break;
                case ExtrioPlugDeviceAction.SET_PREVIEW_SIZE_READY_CALLBACK:
                    mCameraOpenedCallback = (PreviewSizeCallback) msg.obj;
                    break;
                case ExtrioPlugDeviceAction.GET_PREVIEW_SIZE:
                    doGetPreviewSize(msg);
                    break;
                case ExtrioPlugDeviceAction.UPDATE_PREVIEW_SURFACE:
                    doUpdatePreviewSurface(msg.obj);
                    break;
                case ExtrioPlugDeviceAction.UPDATE_G_SENSOR_ORIENTATION:
                    mJpegRotation = (Integer) msg.obj;
                    break;
                case ExtrioPlugDeviceAction.IS_READY_FOR_CAPTURE:
                    boolean[] isReady = (boolean[]) msg.obj;
                    isReady[0] = isReadyForCapture();
                    break;
                case ExtrioPlugDeviceAction.DESTROY_DEVICE_CONTROLLER:
                    doDestroyHandler();
                    break;

                case ExtrioPlugDeviceAction.ON_CAMERA_OPENED:
                    doOnOpened((CameraProxy) msg.obj);
                    break;
                case ExtrioPlugDeviceAction.ON_CAMERA_DISCONNECTED:
                    doOnDisconnected();
                    break;
                case ExtrioPlugDeviceAction.ON_CAMERA_ERROR:
                    doOnError(msg.arg1);
                    break;
                default:
        ...
        private boolean cancelDealMessage(int message) {
            ...
            boolean value = false;
            if (mRequestHandler.hasMessages(ExtrioPlugDeviceAction.CLOSE_CAMERA)) {
                switch (message) {
                    case ExtrioPlugDeviceAction.START_PREVIEW:
                    case ExtrioPlugDeviceAction.STOP_PREVIEW:
                    case ExtrioPlugDeviceAction.TAKE_PICTURE:
        ...
        private boolean hasDeviceStateCallback() {
            boolean value = mRequestHandler.hasMessages(ExtrioPlugDeviceAction.ON_CAMERA_ERROR)
                    || mRequestHandler.hasMessages(ExtrioPlugDeviceAction.ON_CAMERA_CLOSED)
                    || mRequestHandler.hasMessages(ExtrioPlugDeviceAction.ON_CAMERA_DISCONNECTED)
                    || mRequestHandler.hasMessages(ExtrioPlugDeviceAction.ON_CAMERA_OPENED);
        ...
        private class CameraDeviceProxyStateCallback extends CameraProxy.StateCallback {
            ...
            public void onOpened(@Nonnull CameraProxy cameraProxy) {
                ...
                synchronized (mWaitCameraOpenDone) {
                    ...
                    mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.ON_CAMERA_OPENED, cameraProxy)
                            .sendToTarget();
                }
            }
            ...
            public void onDisconnected(@Nonnull CameraProxy cameraProxy) {
                ...
                if (mCameraProxy != null && mCameraProxy == cameraProxy) {
                    synchronized (mWaitCameraOpenDone) {
                        mCameraState = CameraState.CAMERA_UNKNOWN;
                        mWaitCameraOpenDone.notifyAll();
                        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.ON_CAMERA_DISCONNECTED,
            ...
            public void onError(@Nonnull CameraProxy cameraProxy, int error) {
                ...
                if ((mCameraProxy != null && mCameraProxy == cameraProxy)
                    ...
                    synchronized (mWaitCameraOpenDone) {
                        mCameraState = CameraState.CAMERA_UNKNOWN;
                        mWaitCameraOpenDone.notifyAll();
                        mRequestHandler.obtainMessage(ExtrioPlugDeviceAction.ON_CAMERA_ERROR, error, 0,
                                cameraProxy).sendToTarget();
                    }
                }
            }
