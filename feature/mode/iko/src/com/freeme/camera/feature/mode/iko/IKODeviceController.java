 ...
 */
public class IKODeviceController implements IIKODeviceController, SettingDeviceRequester {
    private static final Tag TAG = new Tag(IKODeviceController.class.getSimpleName());
    ...
    private IKOHandler mRequestHandler;
     ...
     */
    IKODeviceController(@Nonnull Activity activity, @Nonnull ICameraContext context) {
        HandlerThread handlerThread = new HandlerThread("DeviceController");
        handlerThread.start();
        mRequestHandler = new IKOHandler(handlerThread.getLooper(), this);
    ...
    public void openCamera(DeviceInfo info) {
        mRequestHandler.obtainMessage(IKODeviceAction.OPEN_CAMERA, info).sendToTarget();
    }

    @Override
    public void updatePreviewSurface(Object surfaceObject) {
        mRequestHandler.obtainMessage(IKODeviceAction.UPDATE_PREVIEW_SURFACE, surfaceObject)
                .sendToTarget();
    }

    @Override
    public void setDeviceCallback(DeviceCallback callback) {
        mRequestHandler.obtainMessage(IKODeviceAction.SET_PREVIEW_CALLBACK, callback)
                .sendToTarget();
    }

    @Override
    public void startPreview() {
        mRequestHandler.sendEmptyMessage(IKODeviceAction.START_PREVIEW);
    }

    @Override
    public void stopPreview() {
        mRequestHandler.sendEmptyMessage(IKODeviceAction.STOP_PREVIEW);
        waitDone();
    }

    @Override
    public void takePicture(@Nonnull JpegCallback callback) {
        mRequestHandler.obtainMessage(IKODeviceAction.TAKE_PICTURE, callback).sendToTarget();
    }

    @Override
    public void updateGSensorOrientation(int orientation) {
        mRequestHandler.obtainMessage(IKODeviceAction.UPDATE_G_SENSOR_ORIENTATION, orientation)
                .sendToTarget();
    }

    @Override
    public void closeCamera(boolean sync) {
        mRequestHandler.obtainMessage(IKODeviceAction.CLOSE_CAMERA, sync == true ? 1 : 0)
                .sendToTarget();
        waitDone();
    }
    ...
    public Size getPreviewSize(double targetRatio) {
        ...
        mRequestHandler.obtainMessage(IKODeviceAction.GET_PREVIEW_SIZE, values).sendToTarget();
        waitDone();
    ...
    public void setPreviewSizeReadyCallback(PreviewSizeCallback callback) {
        mRequestHandler.obtainMessage(IKODeviceAction.SET_PREVIEW_SIZE_READY_CALLBACK,
                callback).sendToTarget();
    }

    @Override
    public void setPictureSize(Size size) {
        mRequestHandler.obtainMessage(IKODeviceAction.SET_PICTURE_SIZE, size).sendToTarget();
    }

    @Override
    public boolean isReadyForCapture() {
        boolean[] isReady = new boolean[1];
        mRequestHandler.obtainMessage(IKODeviceAction.IS_READY_FOR_CAPTURE, isReady)
                .sendToTarget();
        waitDone();
        return isReady[0];
    }
    ...
    public void requestChangeSettingValue(String key) {
        mRequestHandler.removeMessages(IKODeviceAction.REQUEST_CHANGE_SETTING_VALUE);
        mRequestHandler.obtainMessage(IKODeviceAction.REQUEST_CHANGE_SETTING_VALUE, key)
                .sendToTarget();
    }
    ...
    public void requestChangeSettingValueJustSelf(String key) {
        ...
        mRequestHandler.removeMessages(IKODeviceAction
                .REQUEST_CHANGE_SETTING_VALUE_JUST_SELF, key);
        mRequestHandler.obtainMessage(IKODeviceAction.REQUEST_CHANGE_SETTING_VALUE_JUST_SELF,
                key).sendToTarget();
    }

    @Override
    public void requestChangeCommand(String key) {
        mRequestHandler.obtainMessage(IKODeviceAction.REQUEST_CHANGE_COMMAND, key).sendToTarget();
    }

    @Override
    public void requestChangeCommandImmediately(String key) {
        mRequestHandler.obtainMessage(IKODeviceAction.REQUEST_CHANGE_COMMAND_IMMEDIATELY, key)
                .sendToTarget();
    }

    @Override
    public void destroyDeviceController() {
        mRequestHandler.sendEmptyMessage(IKODeviceAction.DESTROY_DEVICE_CONTROLLER);
    }
     ...
     */
    private class IKOHandler extends Handler {
         ...
         */
        public IKOHandler(Looper looper, SettingDeviceRequester requester) {
            super(looper);
        ...
        public void handleMessage(Message msg) {
            ...
            if (cancelDealMessage(msg.what)) {
                LogHelper.d(TAG, "[handleMessage] - msg = " +
                        IKODeviceAction.stringify(msg.what) + "[dismiss]");
                return;
            }
            switch (msg.what) {
                case IKODeviceAction.OPEN_CAMERA:
                    doOpenCamera((DeviceInfo) msg.obj);
                    break;
                case IKODeviceAction.CLOSE_CAMERA:
                    doCloseCamera((Integer) msg.obj == 1 ? true : false);
                    break;
                case IKODeviceAction.START_PREVIEW:
                    doStartPreview();
                    break;
                case IKODeviceAction.STOP_PREVIEW:
                    doStopPreview();
                    break;
                case IKODeviceAction.TAKE_PICTURE:
                    doTakePicture((JpegCallback) msg.obj);
                    break;
                case IKODeviceAction.REQUEST_CHANGE_SETTING_VALUE:
                    ...
                    break;
                case IKODeviceAction.REQUEST_CHANGE_SETTING_VALUE_JUST_SELF:
                    ...
                    break;
                case IKODeviceAction.REQUEST_CHANGE_COMMAND:
                    doRequestChangeCommand((String) msg.obj);
                    break;
                case IKODeviceAction.REQUEST_CHANGE_COMMAND_IMMEDIATELY:
                    doRequestChangeCommandImmediately((String) msg.obj);
                    break;
                case IKODeviceAction.SET_PICTURE_SIZE:
                    doSetPictureSize((Size) msg.obj);
                    break;
                case IKODeviceAction.SET_PREVIEW_CALLBACK:
                    mModeDeviceCallback = (DeviceCallback) msg.obj;
                    break;
                case IKODeviceAction.SET_PREVIEW_SIZE_READY_CALLBACK:
                    mCameraOpenedCallback = (PreviewSizeCallback) msg.obj;
                    break;
                case IKODeviceAction.GET_PREVIEW_SIZE:
                    doGetPreviewSize(msg);
                    break;
                case IKODeviceAction.UPDATE_PREVIEW_SURFACE:
                    doUpdatePreviewSurface(msg.obj);
                    break;
                case IKODeviceAction.UPDATE_G_SENSOR_ORIENTATION:
                    mJpegRotation = (Integer) msg.obj;
                    break;
                case IKODeviceAction.IS_READY_FOR_CAPTURE:
                    boolean[] isReady = (boolean[]) msg.obj;
                    isReady[0] = isReadyForCapture();
                    break;
                case IKODeviceAction.DESTROY_DEVICE_CONTROLLER:
                    doDestroyHandler();
                    break;

                case IKODeviceAction.ON_CAMERA_OPENED:
                    doOnOpened((CameraProxy) msg.obj);
                    break;
                case IKODeviceAction.ON_CAMERA_DISCONNECTED:
                    doOnDisconnected();
                    break;
                case IKODeviceAction.ON_CAMERA_ERROR:
                    doOnError(msg.arg1);
                    break;
                default:
        ...
        private boolean cancelDealMessage(int message) {
            ...
            boolean value = false;
            if (mRequestHandler.hasMessages(IKODeviceAction.CLOSE_CAMERA)) {
                switch (message) {
                    case IKODeviceAction.START_PREVIEW:
                    case IKODeviceAction.STOP_PREVIEW:
                    case IKODeviceAction.TAKE_PICTURE:
        ...
        private boolean hasDeviceStateCallback() {
            boolean value = mRequestHandler.hasMessages(IKODeviceAction.ON_CAMERA_ERROR)
                    || mRequestHandler.hasMessages(IKODeviceAction.ON_CAMERA_CLOSED)
                    || mRequestHandler.hasMessages(IKODeviceAction.ON_CAMERA_DISCONNECTED)
                    || mRequestHandler.hasMessages(IKODeviceAction.ON_CAMERA_OPENED);
        ...
        private class CameraDeviceProxyStateCallback extends CameraProxy.StateCallback {
            ...
            public void onOpened(@Nonnull CameraProxy cameraProxy) {
                ...
                synchronized (mWaitCameraOpenDone) {
                    ...
                    mRequestHandler.obtainMessage(IKODeviceAction.ON_CAMERA_OPENED, cameraProxy)
                            .sendToTarget();
                }
            }
            ...
            public void onDisconnected(@Nonnull CameraProxy cameraProxy) {
                ...
                if (mCameraProxy != null && mCameraProxy == cameraProxy) {
                    synchronized (mWaitCameraOpenDone) {
                        mCameraState = CameraState.CAMERA_UNKNOWN;
                        mWaitCameraOpenDone.notifyAll();
                        mRequestHandler.obtainMessage(IKODeviceAction.ON_CAMERA_DISCONNECTED,
            ...
            public void onError(@Nonnull CameraProxy cameraProxy, int error) {
                ...
                if ((mCameraProxy != null && mCameraProxy == cameraProxy)
                    ...
                    synchronized (mWaitCameraOpenDone) {
                        mCameraState = CameraState.CAMERA_UNKNOWN;
                        mWaitCameraOpenDone.notifyAll();
                        mRequestHandler.obtainMessage(IKODeviceAction.ON_CAMERA_ERROR, error, 0,
                                cameraProxy).sendToTarget();
                    }
                }
            }
