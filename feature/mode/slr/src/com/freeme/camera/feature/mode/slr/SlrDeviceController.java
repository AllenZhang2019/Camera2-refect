 ...
 */
public class SlrDeviceController implements ISlrDeviceController, SettingDeviceRequester {
    private static final Tag TAG = new Tag(SlrDeviceController.class.getSimpleName());
     ...
     */
    SlrDeviceController(@Nonnull Activity activity, @Nonnull ICameraContext context) {
    ...
    public void openCamera(DeviceInfo info) {
        mRequestHandler.obtainMessage(SlrDeviceAction.OPEN_CAMERA, info).sendToTarget();
    }
    ...
    public void updatePreviewSurface(Object surfaceObject) {
        mRequestHandler.obtainMessage(SlrDeviceAction.UPDATE_PREVIEW_SURFACE, surfaceObject)
                .sendToTarget();
    }

    @Override
    public void setDeviceCallback(DeviceCallback callback) {
        mRequestHandler.obtainMessage(SlrDeviceAction.SET_PREVIEW_CALLBACK, callback)
                .sendToTarget();
    }

    @Override
    public void startPreview() {
        mRequestHandler.sendEmptyMessage(SlrDeviceAction.START_PREVIEW);
    }

    @Override
    public void stopPreview() {
        mRequestHandler.sendEmptyMessage(SlrDeviceAction.STOP_PREVIEW);
        waitDone();
    }

    @Override
    public void takePicture(@Nonnull JpegCallback callback) {
        mRequestHandler.obtainMessage(SlrDeviceAction.TAKE_PICTURE, callback).sendToTarget();
    }

    @Override
    public void updateGSensorOrientation(int orientation) {
        mRequestHandler.obtainMessage(SlrDeviceAction.UPDATE_G_SENSOR_ORIENTATION, orientation)
                .sendToTarget();
    }

    @Override
    public void closeCamera(boolean sync) {
        mRequestHandler.obtainMessage(SlrDeviceAction.CLOSE_CAMERA, sync == true ? 1 : 0)
                .sendToTarget();
        waitDone();
    }
    ...
    public Size getPreviewSize(double targetRatio) {
        ...
        mRequestHandler.obtainMessage(SlrDeviceAction.GET_PREVIEW_SIZE, values).sendToTarget();
        waitDone();
    ...
    public void setPreviewSizeReadyCallback(PreviewSizeCallback callback) {
        mRequestHandler.obtainMessage(SlrDeviceAction.SET_PREVIEW_SIZE_READY_CALLBACK,
                callback).sendToTarget();
    }

    @Override
    public void setPictureSize(Size size) {
        mRequestHandler.obtainMessage(SlrDeviceAction.SET_PICTURE_SIZE, size).sendToTarget();
    }

    @Override
    public boolean isReadyForCapture() {
        boolean[] isReady = new boolean[1];
        mRequestHandler.obtainMessage(SlrDeviceAction.IS_READY_FOR_CAPTURE, isReady)
                .sendToTarget();
        waitDone();
        return isReady[0];
    }
    ...
    public void requestChangeSettingValue(String key) {
        mRequestHandler.removeMessages(SlrDeviceAction.REQUEST_CHANGE_SETTING_VALUE);
        mRequestHandler.obtainMessage(SlrDeviceAction.REQUEST_CHANGE_SETTING_VALUE, key)
                .sendToTarget();
    }
    ...
    public void requestChangeSettingValueJustSelf(String key) {
        ...
        mRequestHandler.removeMessages(SlrDeviceAction
                .REQUEST_CHANGE_SETTING_VALUE_JUST_SELF, key);
        mRequestHandler.obtainMessage(SlrDeviceAction.REQUEST_CHANGE_SETTING_VALUE_JUST_SELF,
                key).sendToTarget();
    }

    @Override
    public void requestChangeCommand(String key) {
        mRequestHandler.obtainMessage(SlrDeviceAction.REQUEST_CHANGE_COMMAND, key).sendToTarget();
    }

    @Override
    public void requestChangeCommandImmediately(String key) {
        mRequestHandler.obtainMessage(SlrDeviceAction.REQUEST_CHANGE_COMMAND_IMMEDIATELY, key)
                .sendToTarget();
    }

    @Override
    public void destroyDeviceController() {
        mRequestHandler.sendEmptyMessage(SlrDeviceAction.DESTROY_DEVICE_CONTROLLER);
    }
    ...
    private class QrCodeScanHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (cancelDealMessage(msg.what)) {
                LogHelper.d(TAG, "[handleMessage] - msg = " +
                        SlrDeviceAction.stringify(msg.what) + "[dismiss]");
                return;
            }
            switch (msg.what) {
                case SlrDeviceAction.OPEN_CAMERA:
                    doOpenCamera((DeviceInfo) msg.obj);
                    break;
                case SlrDeviceAction.CLOSE_CAMERA:
                    doCloseCamera((Integer) msg.obj == 1 ? true : false);
                    break;
                case SlrDeviceAction.START_PREVIEW:
                    doStartPreview();
                    break;
                case SlrDeviceAction.STOP_PREVIEW:
                    doStopPreview();
                    break;
                case SlrDeviceAction.TAKE_PICTURE:
                    doTakePicture((JpegCallback) msg.obj);
                    break;
                case SlrDeviceAction.REQUEST_CHANGE_SETTING_VALUE:
                    ...
                    break;
                case SlrDeviceAction.REQUEST_CHANGE_SETTING_VALUE_JUST_SELF:
                    ...
                    break;
                case SlrDeviceAction.REQUEST_CHANGE_COMMAND:
                    doRequestChangeCommand((String) msg.obj);
                    break;
                case SlrDeviceAction.REQUEST_CHANGE_COMMAND_IMMEDIATELY:
                    doRequestChangeCommandImmediately((String) msg.obj);
                    break;
                case SlrDeviceAction.SET_PICTURE_SIZE:
                    doSetPictureSize((Size) msg.obj);
                    break;
                case SlrDeviceAction.SET_PREVIEW_CALLBACK:
                    mModeDeviceCallback = (DeviceCallback) msg.obj;
                    break;
                case SlrDeviceAction.SET_PREVIEW_SIZE_READY_CALLBACK:
                    mCameraOpenedCallback = (PreviewSizeCallback) msg.obj;
                    break;
                case SlrDeviceAction.GET_PREVIEW_SIZE:
                    doGetPreviewSize(msg);
                    break;
                case SlrDeviceAction.UPDATE_PREVIEW_SURFACE:
                    doUpdatePreviewSurface(msg.obj);
                    break;
                case SlrDeviceAction.UPDATE_G_SENSOR_ORIENTATION:
                    mJpegRotation = (Integer) msg.obj;
                    break;
                case SlrDeviceAction.IS_READY_FOR_CAPTURE:
                    boolean[] isReady = (boolean[]) msg.obj;
                    isReady[0] = isReadyForCapture();
                    break;
                case SlrDeviceAction.DESTROY_DEVICE_CONTROLLER:
                    doDestroyHandler();
                    break;

                case SlrDeviceAction.ON_CAMERA_OPENED:
                    doOnOpened((CameraProxy) msg.obj);
                    break;
                case SlrDeviceAction.ON_CAMERA_DISCONNECTED:
                    doOnDisconnected();
                    break;
                case SlrDeviceAction.ON_CAMERA_ERROR:
                    doOnError(msg.arg1);
                    break;
                default:
        ...
        private boolean cancelDealMessage(int message) {
            ...
            boolean value = false;
            if (mRequestHandler.hasMessages(SlrDeviceAction.CLOSE_CAMERA)) {
                switch (message) {
                    case SlrDeviceAction.START_PREVIEW:
                    case SlrDeviceAction.STOP_PREVIEW:
                    case SlrDeviceAction.TAKE_PICTURE:
        ...
        private boolean hasDeviceStateCallback() {
            boolean value = mRequestHandler.hasMessages(SlrDeviceAction.ON_CAMERA_ERROR)
                    || mRequestHandler.hasMessages(SlrDeviceAction.ON_CAMERA_CLOSED)
                    || mRequestHandler.hasMessages(SlrDeviceAction.ON_CAMERA_DISCONNECTED)
                    || mRequestHandler.hasMessages(SlrDeviceAction.ON_CAMERA_OPENED);
        ...
        private class CameraDeviceProxyStateCallback extends CameraProxy.StateCallback {
            ...
            public void onOpened(@Nonnull CameraProxy cameraProxy) {
                ...
                synchronized (mWaitCameraOpenDone) {
                    ...
                    mRequestHandler.obtainMessage(SlrDeviceAction.ON_CAMERA_OPENED, cameraProxy)
                            .sendToTarget();
                }
            }
            ...
            public void onDisconnected(@Nonnull CameraProxy cameraProxy) {
                ...
                if (mCameraProxy != null && mCameraProxy == cameraProxy) {
                    synchronized (mWaitCameraOpenDone) {
                        mCameraState = CameraState.CAMERA_UNKNOWN;
                        mWaitCameraOpenDone.notifyAll();
                        mRequestHandler.obtainMessage(SlrDeviceAction.ON_CAMERA_DISCONNECTED,
            ...
            public void onError(@Nonnull CameraProxy cameraProxy, int error) {
                ...
                if ((mCameraProxy != null && mCameraProxy == cameraProxy)
                    ...
                    synchronized (mWaitCameraOpenDone) {
                        mCameraState = CameraState.CAMERA_UNKNOWN;
                        mWaitCameraOpenDone.notifyAll();
                        mRequestHandler.obtainMessage(SlrDeviceAction.ON_CAMERA_ERROR, error, 0,
                                cameraProxy).sendToTarget();
                    }
                }
            }
