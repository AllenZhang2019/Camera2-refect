...
package com.freeme.camera.common.mode.photo;
...
import com.freeme.camera.common.exif.Exif;
...
import com.freeme.camera.common.mode.photo.device.DeviceControllerFactory;
import com.freeme.camera.common.mode.photo.device.IDeviceController;
...
import com.freeme.camera.common.storage.MediaSaver;
...
import com.freeme.camera.common.mode.photo.device.DeviceControllerFactory;
import com.freeme.camera.common.mode.photo.device.IDeviceController;
import com.freeme.camera.common.mode.photo.device.IDeviceController.DataCallbackInfo;
import com.freeme.camera.common.mode.photo.device.IDeviceController.DeviceCallback;
import com.freeme.camera.common.mode.photo.device.IDeviceController.JpegCallback;
import com.freeme.camera.common.mode.photo.device.IDeviceController.PreviewSizeCallback;
import com.freeme.camera.common.relation.StatusMonitor;
 ...
 */
public class PhotoMode extends CameraModeBase implements IDeviceController.JpegCallback,
        IDeviceController.DeviceCallback, IDeviceController.PreviewSizeCallback, IMemoryManager.IMemoryListener {
...
//        }
    ...
    public void onDataReceived(IDeviceController.DataCallbackInfo dataCallbackInfo) {
        ...
        if (mIsResumed) {
            ...
            if (mCameraApi == CameraDeviceManagerFactory.CameraApi.API1) {
    ...
    public void onPostViewCallback(byte[] data) {
        ...
        if (data != null && mIsResumed) {
            ...
            ISettingManager.SettingController controller = getSettingManager().getSettingController();
            String mirrorCapturede = controller.queryValue(KEY_MIRROR_CAPTURE);
            if (mCameraId.equals(FRONT_CAMERA_ID) && mPrevMirrorCapture.equals(mirrorCapturede)) {
    ...
    private List<AsyncTask<Void, Void, byte[]>> mTaskExcuteList = new ArrayList<AsyncTask<Void, Void, byte[]>>();
    ...
    private void saveData(byte[] jpegData) {
        if (jpegData != null) {
            ...
            String mirrorCapturede = controller.queryValue(KEY_MIRROR_CAPTURE);
            if (mCameraId.equals(FRONT_CAMERA_ID) && mPrevMirrorCapture.equals(mirrorCapturede)) {
                jpegData = mirrorJpegData(jpegData, 0);
            }
    ...
    byte[] mirrorJpegData(byte[] jpegData, int orientation) {
        byte[] dest = jpegData;
