...
public class CaptureSurface {
    ...
    public CaptureSurface() {
        ...
        HandlerThread mCaptureHandlerThread = new HandlerThread("cap_surface");
     ...
     */
    public void updatePictureInfo(int width, int height, int format, int maxImage) {
        ...
        if (mCaptureImageReader != null && mPictureWidth == width && mPictureHeight == height &&
                format == mFormat && maxImage
                == mMaxImages) {
            LogHelper.d(TAG, "[updatePictureInfo],the info : " + width + " x " + height + "," +
                    "format = " + format + ",maxImage = " + maxImage + " is " +
                    "same as before");
            return;
        }
