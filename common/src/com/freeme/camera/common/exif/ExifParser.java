...
class ExifParser {
    ...
    private static final Tag TAG = new Tag(ExifParser.class.getSimpleName());
    /**
    ...
    protected int next() throws IOException, ExifInvalidFormatException {
        ...
        if (offset < endOfTags) {
            ...
            if (mIfdType == IfdId.TYPE_IFD_0) {
                ...
                if (offsetSize < 4) {
                    LogHelper.w(TAG, "Invalid size of link to next IFD: " + offsetSize);
                } else {
                    long ifdOffset = readUnsignedLong();
                    if (ifdOffset != 0) {
                        LogHelper.w(TAG, "Invalid link to next IFD: " + ifdOffset);
                    }
                }
            }
        }
        while (mCorrespondingEvent.size() != 0) {
            ...
            try {
                ...
                LogHelper.w(TAG, "Failed to skip to data at: " + entry.getKey() +
                        " for " + event.getClass().getName() + ", the file may be broken.");
                continue;
            }
            if (event instanceof IfdEvent) {
                mIfdType = ((IfdEvent) event).mIfd;
                mNumOfTagInIfd = mTiffStream.readUnsignedShort();
                mIfdStartOffset = entry.getKey();

                if (mNumOfTagInIfd * TAG_SIZE + mIfdStartOffset + OFFSET_SIZE > mApp1End) {
                    LogHelper.w(TAG, "Invalid size of IFD " + mIfdType);
                    return EVENT_END;
                }

                mNeedToParseOffsetsInCurrentIfd = needToParseOffsetsInCurrentIfd();
                if (((IfdEvent) event).mIsRequested) {
    ...
    protected int getStripIndex() {
        return mImageEvent.mStripIndex;
    }
    ...
    private ExifTag readTag() throws IOException, ExifInvalidFormatException {
        ...
        if (!ExifTag.isValidType(dataFormat)) {
            LogHelper.w(TAG, String.format("Tag %04x: Invalid data type %d", tagId, dataFormat));
            mTiffStream.skip(4);
            return null;
        }
            ...
            if (mCorrespondingEvent.size() > 0) {
                if (mCorrespondingEvent.firstEntry().getKey() < mTiffStream.getReadByteCount()
                    ...
                    if (event instanceof ImageEvent) {
                        ...
                        LogHelper.w(TAG, "Thumbnail overlaps value for tag: \n" + tag.toString());
                        Entry<Integer, Object> entry = mCorrespondingEvent.pollFirstEntry();
                        LogHelper.w(TAG, "Invalid thumbnail offset: " + entry.getKey());
                    } else {
                        // Tag value overlaps another tag, shorten count
                        if (event instanceof IfdEvent) {
                            LogHelper.w(TAG, "Ifd " + ((IfdEvent) event).mIfd
                                    + " overlaps value for tag: \n" + tag.toString());
                        } else if (event instanceof ExifTagEvent) {
                            LogHelper.w(TAG, "Tag value for tag: \n"
                                    + ((ExifTagEvent) event).mTag.toString()
                                    + " overlaps value for tag: \n" + tag.toString());
                        }
                        size = mCorrespondingEvent.firstEntry().getKey()
                                - mTiffStream.getReadByteCount();
                        LogHelper.w(TAG, "Invalid size of tag: \n" + tag.toString()
    ...
    private boolean seekTiffData(InputStream inputStream) throws IOException,
        ...
        while (marker != JpegHeader.EOI
            ...
            if (length < 2 || (length - 2) != dataStream.skip(length - 2)) {
                LogHelper.w(TAG, "Invalid JPEG format.");
                return false;
            }
    ...
    private static class IfdEvent {
        ...
        IfdEvent(int ifd, boolean isInterestedIfd) {
            this.mIfd = ifd;
            this.mIsRequested = isInterestedIfd;
        }
    }

    /**
     * The ExifTagEvent class.
     */
    private static class ExifTagEvent {
        ...
        ExifTagEvent(ExifTag tag, boolean isRequireByUser) {
            this.mTag = tag;
            this.mIsRequested = isRequireByUser;
        }
    }

    /**
     * Gets the byte order of the current InputStream.
     * @return the byte order of the current InputStream.
     */
