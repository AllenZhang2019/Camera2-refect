...
/**
 * A class used to modify exif.
 */
class ExifModifier {
    private static final Tag TAG = new Tag(ExifModifier.class.getSimpleName());
    ...
    /**
     * A class used to describe Tag offset.
     */
    ...
    private void writeTagValue(ExifTag tag, int offset) {
        if (DEBUG) {
            LogHelper.v(TAG, "modifying tag to: \n" + tag.toString());
            LogHelper.v(TAG, "at offset: " + offset);
        }
