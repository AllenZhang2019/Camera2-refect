 ...
 */
package com.freeme.camera.common.relation;

import java.util.Map;
import java.util.concurrent.ConcurrentHashMap;

/**
 * Factory to create {@link StatusMonitor} object.
 */

public class StatusMonitorFactory {
    private final Map<String/*camera id*/, StatusMonitor> mStatusMonitors
    ...
    public StatusMonitor getStatusMonitor(String cameraId) {
        StatusMonitor monitor = mStatusMonitors.get(cameraId);
        if (monitor == null) {
            monitor = new StatusMonitor();
            mStatusMonitors.put(cameraId, monitor);
        }
        return monitor;
    }
}
