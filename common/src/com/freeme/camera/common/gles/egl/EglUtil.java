 ...
 */
package com.freeme.camera.common.gles.egl;
...
public class EglUtil {
    /**
     * Check whether there is egl error, if has throw runtime exception with op.
     * @param op operation want to check egl error.
     */
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN_MR1)
    public static void checkEglError(String op) {
        int error;
        while ((error = EGL14.eglGetError()) != EGL14.EGL_SUCCESS) {
            throw new RuntimeException(op + "eglGetError:0x" + Integer.toHexString(error));
        }
    }
}